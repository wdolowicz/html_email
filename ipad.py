...
def edit_ipad(version, textfile):
    ...
    file = abspath('ipad_data/letter.html')
    ...
    with open("ipad_data/letter_mod.html", "wb") as file:
        file.write(html)
    changelog_text.close()


def send_ipad(version, me, password, you):
    # Utworzenie zawartości wiadomości
    msg = MIMEMultipart('related')
    msg['Subject'] = str("Wydanie Google for iPad " + version)
    msg['From'] = me
    msg['To'] = you
    msg['CC'] = me

    # Import ciała wiadomości z HTML
    file = abspath('ipad_data/letter_mod.html')
    ...
    s.quit()
    os.remove("ipad_data/letter_mod.html")
    os.remove("temp.txt")
