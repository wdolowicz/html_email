...
def edit_ands(version, textfile):
    ...
    file = abspath('ands_data/letter.html')
    ...
    with open("ands_data/letter_mod.html", "wb") as file:
        file.write(html)
    changelog_text.close()


def send_ands(version, me, password, you):
    # Utworzenie zawartości wiadomości
    msg = MIMEMultipart('related')
    msg['Subject'] = str("Wydanie Google for Android " + version)
    msg['From'] = me
    msg['To'] = you

    # Import ciała wiadomości z HTML
    file = abspath('ands_data/letter_mod.html')
    ...
    s.quit()
    os.remove("ands_data/letter_mod.html")
    os.remove("temp.txt")
